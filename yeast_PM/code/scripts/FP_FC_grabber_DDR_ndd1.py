...
def query_filter(unfiltered_l):
    ...
    for k,v in unfiltered_l.items(): 
        if v.startswith("FP { 0, 1") and v.endswith("0, 1 }"):  
            filtered_l[v] = [k] 
        else: 
            pass
    return filtered_l
...
def fp_queries(db):
    ...
    '''
    if os.path.exists("bistable_fp_query_DDR_ndd1.json") and os.path.exists("multistable_fp_query_DDR_ndd1.json"):
        multistable_fp_filtered =  json.load(open("multistable_fp_query_DDR_ndd1.json"))
        bistable_fp_filtered = json.load(open("bistable_fp_query_DDR_ndd1.json"))
    else:
        ...
        with open("bistable_fp_query_DDR_ndd1.json", "w") as f:
            json.dump(bistable_fp_filtered, f)
        multistable_fp_filtered = query_filter(multistable_fp_dict)
        with open("multistable_fp_query_DDR_ndd1.json", "w") as f:
