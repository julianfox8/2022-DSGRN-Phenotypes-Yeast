...
def query(network_file,params_file,resultsdir="",poset_network_file=None):
    '''
    ...
    poset_spec = read_networks(poset_network_file)
    posets,networks = get_posets(poset_spec,param_dict)
...
def stableFC_check(domaingraph,patterngraph):
    ...
    for i in range(0, morsegraph.poset().size()):
        if morsegraph.annotation(i)[0] == "XC {SWI4, NRM1, NDD1, ACE2}" and len(morsegraph.poset().children(i)) == 0:
            FC = True
            searchgraph = DSGRN.SearchGraph(domaingraph, i, ["CLB2"])
            matchinggraph = DSGRN.MatchingGraph(searchgraph, patterngraph)
            if DSGRN.PathMatch(matchinggraph):
                ismatch = True
                break
...
if __name__ == "__main__":
    ...
    if len(sys.argv)>3:
        ...
        poset_network_file = sys.argv[4]
        query(network_file, params_file, resultsdir, poset_network_file)
    else:
