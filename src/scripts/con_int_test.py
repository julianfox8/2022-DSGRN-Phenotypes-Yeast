...
def param_transition(wt_pm_list, xc_pm_dict, network, fg_label, pheno, noise, resultdir=''):
    ...
    keys = set(clb2_xc).intersection(set(clb2_con.keys()))
    xc_pm = {k:clb2_con[k] for k in keys}
    pm_dict[noise] = xc_pm
    print("Test completed: \n where {} matches were found between reconstructed WT parameters and XC parameters\n\n".format(pheno,noise,len(xc_pm)))


if __name__ == '__main__':
    wt_pm_list = sys.argv[1]
    xc_pm_dict = sys.argv[2]
    network = sys.argv[3]
    fg_label = sys.argv[4]
    pheno = sys.argv[5]
    resultdir = sys.argv[6]
    param_transition(wt_pm_list, xc_pm_dict, network, fg_label, pheno, resultdir)
