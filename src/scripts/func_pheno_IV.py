...
def mutant_hex_order(net,hi_pm,lo_pm,int_hi_pm,int_lo_pm):
    ...
    hi_hex_order = hex_order_grabber(net,hi_plist[net_spec[0]][-1][1])
    print(len(hi_hex_order))
    del hi_plist
    lo_plist = json.load(open(lo_pm))
    lo_hex_order = hex_order_grabber(net,lo_plist[net_spec[0]][-1][1])
    ...
    int_hi_hex_order = hex_order_grabber(net,int_hi_plist[net_spec[0]][-1][1])
    ...
    int_lo_hex_order = hex_order_grabber(net,int_lo_plist[net_spec[0]][-1][1])
